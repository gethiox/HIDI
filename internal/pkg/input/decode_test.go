...
func TestDecoding(t *testing.T) {
	data, err := os.ReadFile("./test_data/devices")
	assert.Empty(t, err)

	devices, err := unmarshal(data)
	assert.Empty(t, err)
	...
	t.Run("Handler count", func(t *testing.T) {
		assert.Equal(t, len(expected), len(devices))
	})

	for i := 0; i < len(expected); i++ {
		expectedDeviceInfo := expected[i]
		actualDeviceInfo := devices[i]

		t.Run(expectedDeviceInfo.Name, func(t *testing.T) {
			assert.Equal(t, expectedDeviceInfo, actualDeviceInfo)
		})
	}
}

func TestDecoding_EmptyFile(t *testing.T) {
	data, err := os.ReadFile("./test_data/devices_empty")
	assert.Empty(t, err)
