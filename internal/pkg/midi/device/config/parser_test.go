...
func TestParseDefaultKeyboard(t *testing.T) {
	...
	expectedConfig := Config{
		...
		KeyMappings: []KeyMapping{
			{
				Name: "Piano",
				Midi: map[string]map[evdev.EvCode]Key{
					"": {
				...
				Analog:          map[string]map[evdev.EvCode]Analog{},
				Deadzones:       map[string]map[evdev.EvCode]float64{},
				DefaultDeadzone: map[string]float64{},
			},
			{
				Name: "Chromatic",
				Midi: map[string]map[evdev.EvCode]Key{
...
func TestParseGamepadDeadzoneAtCenter(t *testing.T) {
	data, err := os.ReadFile("../../../../../cmd/hidi/hidi-config/factory/gamepad/PS4_Controller.toml")
	assert.Equal(t, nil, err)
		...
		KeyMappings: []KeyMapping{
			{
				...
				Analog: map[string]map[evdev.EvCode]Analog{
					"": {
						evdev.ABS_X:     {MappingType: AnalogCC, CC: 0, DeadzoneAtCenter: true},
						evdev.ABS_Y:     {MappingType: AnalogPitchBend, FlipAxis: true, DeadzoneAtCenter: true},
						evdev.ABS_RX:    {MappingType: AnalogCC, CC: 1, CCNeg: 2, Bidirectional: true, DeadzoneAtCenter: true},
						evdev.ABS_RY:    {MappingType: AnalogCC, CC: 3, CCNeg: 4, FlipAxis: true, Bidirectional: true, DeadzoneAtCenter: true},
						evdev.ABS_Z:     {MappingType: AnalogCC, CC: 5},
						evdev.ABS_RZ:    {MappingType: AnalogCC, CC: 6},
						evdev.ABS_HAT0X: {MappingType: AnalogActionSim, Action: OctaveUp, ActionNeg: OctaveDown, Bidirectional: true},
						evdev.ABS_HAT0Y: {MappingType: AnalogActionSim, Action: MappingUp, ActionNeg: MappingDown, FlipAxis: true, Bidirectional: true},
					...
					"Motion Sensors": {
						evdev.ABS_X:  {MappingType: AnalogCC, CC: 7},
						evdev.ABS_Y:  {MappingType: AnalogCC, CC: 8},
						evdev.ABS_Z:  {MappingType: AnalogCC, CC: 9},
						evdev.ABS_RX: {MappingType: AnalogCC, CC: 10},
						evdev.ABS_RY: {MappingType: AnalogCC, CC: 11},
						evdev.ABS_RZ: {MappingType: AnalogCC, CC: 12},
					},
					"Touchpad": {
						evdev.ABS_X: {MappingType: AnalogCC, CC: 13},
						evdev.ABS_Y: {MappingType: AnalogCC, CC: 14},
					},
				},
				Deadzones: map[string]map[evdev.EvCode]float64{
					"": {
